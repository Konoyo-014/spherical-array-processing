...
- **Ambisonic encoding** — typed `AmbisonicSpec` / `AmbisonicFrame` containers, plane-wave source encoding (`ambi.encode_plane_wave`, `ambi.encode_plane_wave_frame`), dense order/channel helpers for ACN/FuMa labels, mixed-order masks, channel activity, channel energy, and **signal health reports** (`ambi.per_order_energy`, `ambi.ambisonic_signal_report`), Tikhonov and WNG-limited inverse filters (`encoding.radial_equalizer`) for turning raw microphone SHT into plane-wave-steering signals, plus **measured-steering-matrix encoders** (`encoding.measured_array_equalizer`, regLS / regLSHD) with `encoding.measured_array_diagnostics` for condition, WNG, gain, and reconstruction-error reporting.
- **Ambisonic decoding** — `sad`, `mmd` / `mad`, `epad`, and `allrad` (VBAP-based) loudspeaker decoders plus the unified `decoder_matrix` entry point, **dual-band `(D_lf, D_hf)` decoders** with max-rE tapering (`dual_band_decoder_matrix` / `apply_dual_band_decoder`), general **frequency-dependent decoder banks** (`frequency_dependent_decoder_matrix`) with `max_re` / `in_phase` / no-taper policies, layout helpers (`layout_itu_5_1`, `layout_itu_7_1_4`, `layout_t_design`), **decoder diagnostics** for rank, condition, mode matching, energy preservation, mode leakage, speaker load, probe energy vectors, and probe velocity vectors, and **imaginary-loudspeaker hull closure** (`suggest_imaginary_loudspeakers`) for hemispherical / partial layouts.
- **Measurement, calibration, and interop** — measured-array transfer-function containers, gain/phase mismatch diagnostics, steering-matrix conditioning, regularized inverse-bank helpers, rigid alignment / fitted-sphere calibration, common loudspeaker layouts, and JSON-safe interchange for grids, basis specs, array geometries, loudspeaker layouts, and measured transfers.
...
- **Acoustics** — spherical Bessel, Neumann, and Hankel functions, wavenumber helpers, modal coefficients for open/rigid/cardioid/directional arrays, SPL/SIL/PWL conversions, fractional-octave bands, A/C/Z weighting, plane-wave impedance relations, phase/delay, Doppler, Mel/ERB/Bark, phon/sone, and classical band-Q helpers.
...
| `sap.acoustics`    | `besseljs`, `besseljsd`, `besselys`, `besselysd`, `besselhs`, `besselhsd`, `besselhs2`, `besselhs2d`, `wavenumber`, `kr`, `plane_wave_radial_bn` (open/rigid/cardioid/directional with `dir_coeff`), `bn_matrix`, `sph_modal_coeffs`, `pressure_to_spl`, `leq`, `intensity_to_sound_intensity_level`, `sound_power_to_sound_power_level`, `fractional_octave_bands`, `a_weighting_db`, `mel_rate`, `erb_rate`, `phon_to_sone`, `sabine_rt60`, `eyring_rt60`, `millington_sette_rt60`, `arau_puchades_rt60`, `schroeder_frequency`, `critical_distance`, `rectangular_room_modes`, `air_absorption_coefficient_iso9613` |
...
| `sap.decoding`     | `DecoderConfig`, `save_decoder_config`, `load_decoder_config`, `decoder_matrix`, `apply_decoder`, `sad_decoder`, `mmd_decoder`, `epad_decoder`, `allrad_decoder`, `vbap_gains`, `layout_from_directions`, `layout_from_directions_deg`, `layout_t_design`, `layout_itu_5_1`, `layout_itu_7_1_4`, `suggest_imaginary_loudspeakers`, `check_layout_coverage`, `decoder_diagnostics`, `decoder_health_report`, `mode_matching_error`, `decoder_energy_preservation_error`, `decoder_mode_leakage_ratio`, `probe_energy_vector`, `probe_velocity_vector`, `dual_band_decoder_matrix`, `apply_dual_band_decoder`, `frequency_dependent_decoder_matrix`, `decoder_taper_weights`, `apply_decoder_taper`, `max_re_sh_weights`, `in_phase_sh_weights` |
...
| `sap.ambi`         | `AmbisonicSpec`, `AmbisonicFrame`, `AmbisonicSignalReport`, `channel_count`, `infer_order`, `channel_labels`, `channel_label_to_acn`, `mixed_order_mask`, `active_channel_indices`, `channel_energy`, `channel_rms`, `reorder_acn_to_fuma`, `reorder_fuma_to_acn`, `order_channel_slices`, `order_channel_mask`, `per_order_energy`, `ambisonic_signal_report`, `convert_ambi_normalization`, `acn_to_fuma`, `fuma_to_acn`, `read_ambix_wav`, `write_ambix_wav`, `read_ambix_frame`, `write_ambix_frame`, `nfc_hoa_distance_filter`, `encode_plane_wave`, `encode_plane_wave_frame`, `uhj_encode`, `uhj_decode`, `intensity_vector`, `doa_from_intensity`, `translate_foa` |
| `sap.measurement`  | `ArrayMeasurement`, `MeasurementDiagnostics`, `transfer_magnitude_db`, `transfer_group_delay_s`, `normalize_transfer`, `capsule_gain_mismatch_db`, `capsule_phase_mismatch_rad`, `steering_condition_numbers`, `measurement_diagnostics`, `regularized_inverse_bank` |
| `sap.calibration`  | `fit_sphere`, `kabsch_alignment`, `align_points`, `array_geometry_from_points`, `sensor_position_error`, `pairwise_distance_error`, `calibration_report` |
| `sap.layouts`      | `LoudspeakerLayout`, `layout_from_degrees`, `layout_to_degrees`, `layout_cartesian`, `stereo_layout`, `itu_5_1_layout`, `itu_7_1_4_layout`, `itu_5_1_4_layout`, `cube_layout`, `octahedral_layout`, `layout_registry`, `get_layout` |
| `sap.metrics`      | `angular_error`, `angular_position_errors`, `pairwise_angular_distance_matrix`, `assignment_error`, `energy_vector`, `velocity_vector`, `spectral_distortion_db`, `phase_error_rad`, `relative_error`, `scale_invariant_error_db` |
| `sap.interop`      | `grid_to_dict`, `grid_from_dict`, `sh_basis_spec_to_dict`, `sh_basis_spec_from_dict`, `array_geometry_to_dict`, `array_geometry_from_dict`, `loudspeaker_layout_to_dict`, `measurement_to_dict`, `write_object`, `read_object` |
