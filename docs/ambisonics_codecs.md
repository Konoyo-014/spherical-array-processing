...
`epad`, and `allrad`. The matrix is applied with `decoding.apply_decoder`.
Layouts are ordinary `SphericalGrid` objects, so the same coordinate convention
rules used by the SH module apply here.
...
is a calibration-data inversion problem.
